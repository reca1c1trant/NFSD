...
def load_nt_dataset_emb(field="neutron", dataset="iter1", n_data_set=None, device="cuda"):
    folder_path = ABSOLUTE_PATH + "/data/NTcouple/" + dataset
    if field == "neutron":
        cond1 = torch.tensor(np.load(folder_path + "/bc_neu.npy")[0:n_data_set]).float().to(device)  # b,1,t,ny,1
        cond1 = normalize(cond1, "neutron")
        cond2 = torch.tensor(np.load(folder_path + "/fuel_neu.npy")[0:n_data_set]).float().to(device)  # b,1,t,ny,nx
        cond2 = normalize(cond2, "solid")
        cond3 = torch.tensor(np.load(folder_path + "/fluid_neu.npy")[0:n_data_set]).float().to(device)  # b,1,t,ny,nx
        cond3 = normalize(cond3, "fluid")
        cond4 = torch.concat((cond2, cond3), dim=-1)
        data = torch.tensor(np.load(folder_path + "/neu.npy")[0:n_data_set]).float().to(device)
        data = normalize(data, "neutron")
        cond = [cond1, cond4]
    elif field == "solid":
        cond1 = torch.tensor(np.load(folder_path + "/neu_fuel.npy")[0:n_data_set]).float().to(device)  # b,1,t,ny,nx
        cond1 = normalize(cond1, "neutron")
        cond2 = torch.tensor(np.load(folder_path + "/fluid_fuel.npy")[0:n_data_set]).float().to(device)  # b,1,t,ny,1
        cond2 = normalize(cond2, "fluid")
        data = torch.tensor(np.load(folder_path + "/fuel.npy")[0:n_data_set]).float().to(device)  # b,1,t,ny,nx
        data = normalize(data, "solid")
        cond = [cond1, cond2]
    elif field == "fluid":
        cond1 = torch.tensor(np.load(folder_path + "/fuel_fluid.npy")[0:n_data_set]).float().to(device)  # b,1,t,ny,1
        cond1 = normalize(cond1, "flux")
...
def cond_emb(field="neutron", nx_fuel=8, nx_fluid=12, ny=64, nt=32, hidden=32, device="cuda"):
    def consistent(x):
        return x

    def expand(x: torch.tensor, n):
        return x.expand(-1, -1, -1, -1, n)

    if field == "neutron":
        return [partial(expand, n=20), consistent]
    elif field == "solid":
        return [consistent, partial(expand, n=8)]
    elif field == "fluid":
        return [partial(expand, n=12)]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp_id", default="nuclear_thermal_coupling", type=str, help="experiment folder id")
    parser.add_argument("--train_which", default="neutron", type=str, help="train neutron or solid or fluid")
    ...
    parser.add_argument("--gap", default=32, type=int, help="dataset size for valiadation")
    ...
    device = "cuda"
    cond, data = load_nt_dataset_emb(train_which, args.dataset, args.n_dataset, device=device)
    emb = cond_emb(train_which, device=device)
    interval = -args.gap

    image_size = data.shape[-2:]
    frames = data.shape[2]
    if train_which == "neutron":
        train_dataset = TensorDataset(data[:interval], cond[0][:interval], cond[1][:interval])
        data_val = TensorDataset(data[interval:], cond[0][interval:], cond[1][interval:])
    elif train_which == "solid":
        train_dataset = TensorDataset(data[:interval], cond[0][:interval], cond[1][:interval])
        data_val = TensorDataset(data[interval:], cond[0][interval:], cond[1][interval:])
    elif train_which == "fluid":
