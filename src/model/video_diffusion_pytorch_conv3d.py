...
from src.model.text import tokenize, bert_embed, BERT_MODEL_DIM
...
class MLP(nn.Module):
    ...
    def __init__(self, layers_cfg):
        ...
        self.L = len(layers_cfg)
        assert self.L >= 2
        for i in range(self.L - 1):
            layer = nn.Linear(*layers_cfg[i : i + 2])
            setattr(self, f"layer{i}", layer)

    def forward(self, *args):
        inputs = torch.cat(args, 1)
        for i in range(self.L - 2):
            hidden_layer = getattr(self, f"layer{i}")
            outputs = F.elu(hidden_layer(inputs))
            inputs = outputs
        output_layer = getattr(self, f"layer{self.L-2}")
...
class Unet3D_with_Conv3D(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.channels = out_dim + cond_dim if channels is None else channels
        ...
        # time conditioning
        time_dim = dim * 4 if time_cond is True else None
        self.time_mlp = (
            nn.Sequential(SinusoidalPosEmb(dim), nn.Linear(dim, time_dim), nn.GELU(), nn.Linear(time_dim, time_dim))
            if time_cond is True
            else None
        )
        ...
        # other field conditioning
        # if not isinstance(cond_emb, nn.Module):
        #     self.cond_emb = nn.ModuleList(cond_emb)  # it should be Module class, or it will not save the parameter!
                ...
                nn.ModuleList(
                    [
                        ...
                        (
                            Residual(
                                PreNorm(
                                    dim_out, SpatialLinearAttention(dim_out, heads=attn_heads, dim_head=attn_dim_head)
                                )
                            )
        ...
        spatial_attn = EinopsToAndFrom(
            "b c f h w", "b f (h w) c", Attention(mid_dim, heads=attn_heads, dim_head=attn_dim_head)
        )
    ...
    def forward(
        ...
        # surrogate
            ...
            for i, field in enumerate(x):
                if i == 0:
                    continue
                emb_field = self.cond_emb[i](field)
                # print(x_new.shape, emb_field.shape)
        ...
        t = self.time_mlp(time) if exists(self.time_mlp) and time is not None else None
        h = []
